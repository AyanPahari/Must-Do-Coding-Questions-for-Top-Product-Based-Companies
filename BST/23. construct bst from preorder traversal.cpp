...
Node *solve(Node *&root, int pre[], int size, int &pos, int left, int right)
{
    if (left > right)
        return NULL;
    if (pos >= size)
        return NULL;
    if (pre[pos] < left || pre[pos] > right)
        return NULL;

    int temp = pre[pos];
    root = newNode(pre[pos++]);
    root->left = solve(root->left, pre, size, pos, left, temp - 1);
    root->right = solve(root->right, pre, size, pos, temp + 1, right);
...
Node *constructTree(int pre[], int size)
{
    ...
    int pos = 0;
    solve(root, pre, size, pos, INT_MIN, INT_MAX);
    return root;
}
